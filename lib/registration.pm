...
sub add_suseconnect_product {
    ...
    while ($try_cnt++ <= $retry) {
        if (is_transactional) {
            # Currently only sle-micro6.0/6.1 support Extras, but there is SCC setup issue which is not sorted yet in 6.1
            # We need to wait till it is fixed
            eval { trup_call("register -p $name/" . get_var('VERSION') . '/' . get_var('ARCH')) if (get_var('VERSION') eq '6.0') };
        } else {
